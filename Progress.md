...
Progresso dello sviluppo dell'app Italian Meals, realizzata con Expo / React Native, riusando i pattern visti nei laboratori del corso (Lab 01-18).


Stato generale

LabArgomentoStato01–06Setup progetto, architettura modulare (src/components, src/screens, src/services)✅ Completato07Avatar rotondo con fallback (pattern previsto, da integrare in Login/Settings)⚠️ Componente non ancora creato09–10useState / useEffect, side effects asincroni✅ Applicato in tutte le schermate dati11Form controllati (Login)✅ Completato13–14React Navigation, route params, deep linking✅ Completato (lista → dettaglio → preferiti)15REST calls (fetch) e workflow asincroni con stati UI✅ Completato16Persistenza locale con AsyncStorage (preferiti)✅ Completato17Stato globale con Context (FavoritesContext)✅ Completato18UI styling: StyleSheet, Flexbox, layout responsive✅ Completato
...
src/
  ...
  theme/
    colors.ts                ✅ Token colori e spacing (xs/sm/md/lg) (Lab 18)
    styles.ts                ✅ createSharedStyles() con stili condivisi tra schermate (Lab 18)
  types/
...
6. Styling e layout responsive (Lab 18)


theme/colors.ts: token colori (primary, accent, card: "#fcc9c9", ecc.) e token spaziatura (xs: 4, sm: 8, md: 16, lg: 24) — nessun numero "magico" nei componenti
theme/styles.ts: createSharedStyles() chiamata dentro i componenti — stili condivisi per screen, center, listItem, listItemWide, thumb, thumbWide, title, listTitle, tabsRow, favCount, ecc.
MealsListScreen e FavoritesScreen usano useWindowDimensions() con breakpoint width >= 600
Layout a 1 colonna su mobile, 2 colonne su tablet (numColumns={isWide ? 2 : 1})
key={isWide ? "wide" : "narrow"} sulla FlatList per evitare crash al cambio colonne
numberOfLines={2} su tutti i nomi piatto per gestire testi lunghi
Nessuno stile inline nelle schermate principali
...
Icona profilo in alto a sinistra su MealsListScreen, che porterà a ProfileScreen
ProfileScreen con Avatar (componente Lab 07 da creare) + nome utente da useAuth()
SettingsScreen (correggere il nome file da SetttingScreen.tsx) con logout (useAuth().logout())
Ricerca/filtro testuale sulla lista piatti
Pulizia file orfani (src/HomeScreen.tsx, src/DetailScreen.tsx)
README.md finale con istruzioni di installazione ed esecuzione